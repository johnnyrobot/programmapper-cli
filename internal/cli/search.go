...
In local mode: searches locally synced data only.`,
  ...
  programmapper-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /site-contents/{site_content_id}/programs/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/site-contents/{site_content_id}/programs/search", map[string]any{
					"q":            query,
					"searchString": "",
				})
				if getErr == nil {
			...
			switch resourceType {
			case "site-contents":
				results, err = db.SearchSiteContents(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchSiteContents(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search site_contents failed: %w", searchErr)
					}
