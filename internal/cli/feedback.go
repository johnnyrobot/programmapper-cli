...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PROGRAMMAPPER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PROGRAMMAPPER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "programmapper-pp-cli/feedback")
...
When ` + "`PROGRAMMAPPER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PROGRAMMAPPER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  programmapper-pp-cli feedback list
  programmapper-pp-cli feedback list --limit 5
  programmapper-pp-cli feedback list --json`,
